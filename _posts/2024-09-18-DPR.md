...
따라서 이를 위한 efficient retriever component, 즉 정확한 정답을 찾아내기 전에 query와 유사한 집합 (전체 corpus의 부분집합)을 구할 수 있는 방법이 필요하다. Retriever $R$을 $R : (q, \mathcal{C}) \rightarrow \mathcal{C}_{\mathcal{F}}$, $\mathcal{C}$를 corpus, $q$ 를 question 이라고 했을 때, retriever은 $\mathcal{C}_{\mathcal{F}} \in \mathcal{C},\;\; |\mathcal{C}_{\mathcal{F}}| = k \ll |\mathcal{C}|$ 한 corpus $\mathcal{C}$의 subset인 $\mathcal{C}_{\mathcal{F}}$를 구해내어야 한다.
...
$$ \mathcal{D} = \{\left\langle q*i, p_i^+, p*{i,1}^-, \cdots, p*{i, n}^- \right\rangle\}*{i=1}^m $$
